...
public class DiffdownloaderApplication implements ApplicationRunner {
	...
	public DiffdownloaderApplication(ConfigurableEnvironment environment, ApplicationContext context) {
...
//		((StandardEnvironment) environment).getSystemProperties().put("spring.config.location", configPath);

		DeliveryService deliveryService = (DeliveryService) context.getBean("mainDeliveryService");
		deliveryService.makeZip();
	}
